...
AirPlay.prototype.nokeepaliveplay = function nokeepaliveplay(url, position, cb) {
    if (typeof position === 'function') return this.nokeepaliveplay(url, 0, position)
...
AirPlay.prototype._request = function _request(method, path, body, cb) {
    ...
    req.on('error', function(error) {
        console.log(error)
        var err = new Error('Connection error: ' + error)
    })
    req.on('timeout', function(error) {
        console.log(error)
        var err = new Error('Timeout error: ' + error)
        req.abort();
    });
